...
/**
 * This class is an extension of OthelloWorld that explicitly deals with network
 * games
 * 
 * @author Patrick Lee
 * @version May 26, 2014
...
public class NetworkWorld extends OthelloWorld
{
    ...
    /**
     * Creates a new NetworkWorld with the specified game and default listen
     * port
     * 
     * @param game
     *            NetworkGame
    ...
    /**
     * recieves a method sent from the socket and processes it
     * 
     * @param name
     *            name of the socket
     * @param message
    ...
    public void receive( SocketName name, String message )
    {
        ...
        if ( space == -1 && dot == -1 )
        {
            ...
            setMessage( "Connection successful! Connected to "
                + connections.get( 0 ) + "\n"
                + "You control the black pieces. " + " You move first." );
            networker.send( "name."
                + getGame().getPlayers()[( (NetworkGame)getGame() ).getOwnIndex()].getName() );
        }
        else if ( dot == 4 )
        {
            ...
            setMessage( getMessage()
                ...
                + getGame().getPlayers()[( (NetworkGame)getGame() ).getPlayerIndex()].getName()
                + " moves first." );
        }
        else
        {
    ...
    /**
     * Processes a click on the grid. If it is local, it sends it to the
     * opponent.
     * 
     * @param loc
     *            Location clicked
     * @param local
     *            whether or not the click was local
     */
    public void locationClicked( Location loc, boolean local )
    {
        ...
        if ( ( ( (NetworkGame)getGame() ).getOwnIndex() == getGame().getPlayerIndex() && local )
            || ( (NetworkGame)getGame() ).getOwnIndex() != getGame().getPlayerIndex()
            && !local )
        {
